...
std::string PaymentDisclosureInfo::ToString() const {
    return strprintf("PaymentDisclosureInfo(version=%d, esk=%s, joinSplitPrivKey=%s, address=%s)",
        version, esk.ToString(), joinSplitPrivKey.ToString(), CZCPaymentAddress(zaddr).ToString());
}
...
std::string PaymentDisclosurePayload::ToString() const {
    return strprintf("PaymentDisclosureInfo(version=%d, esk=%s, txid=%s, js=%d, n=%d, address=%s, message=%s)",
