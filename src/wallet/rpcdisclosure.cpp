...
UniValue z_validatepaymentdisclosure(const UniValue& params, bool fHelp)
{
    ...
    if (pd.payload.version != PAYMENT_DISCLOSURE_VERSION_EXPERIMENTAL) {
        errs.push_back("Payment disclosure refers to an unknown version");
    }
    o.push_back(Pair("version", pd.payload.version));

    o.push_back(Pair("onetimeSymmetricKey", pd.payload.esk.ToString()));
    ...
    bool verified = (crypto_sign_verify_detached(pd.payloadSig.data(),
        dataToBeSigned.begin(), 32,
        tx.joinSplitPubKey.begin()) == 0);
    o.push_back(Pair("signatureVerified", verified));
    if (!verified) {
    ...
    bool isValid = verified && errs.empty();
